    ...
    for field in required_fields:
        if field not in chunk:
            ...
            if not isinstance(chunk[field], int) or chunk[field] < 1:
                empty_fields.append(f"Chunk {i}: 'page' is not a positive int (got {chunk[field]!r})")
...
# ─────────────────────────────────────────────────────────────────────
# TEST 4: Page numbers correspond to actual PDF page counts
# ─────────────────────────────────────────────────────────────────────
divider("TEST GROUP 4: Page number validity")

# Build a map of source -> max page count from actual PDFs
pdf_page_counts = {}
for pdf_file in os.listdir(DOCUMENTS_DIR):
    if pdf_file.lower().endswith(".pdf"):
        pdf_path = os.path.join(DOCUMENTS_DIR, pdf_file)
        doc = pymupdf.open(pdf_path)
        pdf_page_counts[pdf_file] = len(doc)
        doc.close()
...
for i, chunk in enumerate(loaded_chunks):
    ...
    if src not in pdf_page_counts:
        unknown_sources.append(f"Chunk {i}: source '{src}' not found in documents/")
...
report(
    "All chunk sources match actual PDF files",
    len(unknown_sources) == 0,
    "\n".join(unknown_sources[:5]) if unknown_sources else "",
)
report(
    "All page numbers are within valid range for their source PDF",
    len(invalid_pages) == 0,
...
report(
    f"Chunks come from multiple PDFs ({len(unique_sources)} sources)",
...
first_pdf = sorted(unique_sources)[0]
